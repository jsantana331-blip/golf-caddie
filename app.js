...
async function init(){
 try{
  ...
  state={version:1,startedAt:new Date().toISOString(),results:{},page:0,activeHole:round.holes[0].n};
  try{const raw=JSON.parse(localStorage.getItem(key)||'null');if(raw?.version===1){state.results=RoundResults.clean(raw.results,round.holes);state.startedAt=typeof raw.startedAt==='string'?raw.startedAt:state.startedAt;state.page=Number.isInteger(raw.page)?raw.page:0;state.activeHole=RoundResults.activeHole(round.holes,state.results,raw.activeHole)}}catch{storageOK=false}
...
function summary(){const s=RoundResults.stats(round.holes,state.results),rel=RoundResults.relativeLabel(s);
 return `<div class="eyebrow">ROUND SUMMARY</div><h1>Your round</h1><p>${esc(round.course)} · ${esc(round.tee)} tees</p><section class="round-totals"><p class="eyebrow">${s.played}/18 holes scored</p><div class="totals">${metric(s.played===18?'Total score':'Score so far',s.played?s.score:'—')}${metric('Relative to par · scored holes',s.played?rel:'—')}${metric('Front nine',totalLabel(round.holes.slice(0,9)))}${metric('Back nine',totalLabel(round.holes.slice(9)))}</div></section><div class="summary-actions">${s.played<round.holes.length?`<button id="back-active" class="primary">Back to Hole ${state.activeHole}</button>`:''}<button id="copy-coach" class="secondary">Copy for Golf Coach</button><p id="copy-status" role="status" aria-live="polite"></p><div id="copy-fallback" hidden><label for="coach-text">Golf Coach report</label><textarea id="coach-text" readonly rows="8"></textarea><button id="select-report" class="secondary">Select report text</button><p>Touch and hold the selected text, choose Copy, then paste into your Golf Coach project.</p></div></div><section class="round-stats"><div class="stat"><div class="summaryrow"><span>Fairways hit / eligible</span><b>${s.fairways} / ${s.eligible}</b></div><p class="muted">${s.teeRecorded}/${s.eligible} tee results recorded</p></div><div class="stat"><div class="summaryrow"><span>GIR</span><b>${s.gir} / 18</b></div><p class="muted">${s.girRecorded}/18 GIR results recorded</p></div><div class="stat"><div class="summaryrow"><span>Putts recorded</span><b>${s.putts}</b></div><p class="muted">${s.puttsRecorded}/18 holes recorded</p></div><div class="stat"><div class="summaryrow"><span>Penalty strokes recorded</span><b>${s.penalties}</b></div><p class="muted">${s.penaltiesRecorded}/18 holes recorded</p></div></section><section class="scorecard-section"><h2>Scorecard</h2><p>Use this for manual entry into TheGrint. Tap a hole to edit. Swipe the table sideways for all columns. — means not recorded.</p><div class="scorecard" tabindex="0" role="region" aria-label="18-hole scorecard"><table><thead><tr>${['Hole','Par','Score','Tee','GIR','Putts','Pen.','Note'].map(v=>`<th scope="col">${v}</th>`).join('')}</tr></thead><tbody>${round.holes.map(h=>{const r=state.results[h.n]||{};return `<tr><th scope="row"><button data-hole="${h.n}">${h.n}</button></th><td>${h.par}</td><td>${r.score??'—'}</td><td>${h.par===3?'N/A':r.tee??'—'}</td><td>${r.gir===undefined?'—':r.gir?'Yes':'No'}</td><td>${r.putts??'—'}</td><td>${r.penalties??'—'}</td><td class="note-cell">${esc(r.note||'—')}</td></tr>`}).join('')}</tbody></table></div></section><section class="new-round-section"><button id="new-round" class="secondary">Start a new round</button><p class="muted">Replaces this device’s current scorecard after confirmation.</p></section>`;
}
function render(){const ps=pages(),p=ps[idx];let body='';
 ...
 }else if(p.type==='game')body=`<div class="eyebrow">BEFORE THE FIRST TEE</div><h1>Game Plan</h1><section class="mission"><h2>Scoring Mission</h2><p class="strategy">${esc(round.mission)}</p></section><div class="briefing">${round.gamePlan.map(([title,text])=>`<section><h2>${esc(title)}</h2><p>${esc(text)}</p></section>`).join('')}</div><section class="club-distances"><h2>Your club distances</h2><div class="clubgrid">${round.clubs.map(([c,y])=>`<div class="club"><b>${esc(c)}</b><span>${y} yd</span></div>`).join('')}</div></section>`;
 ...
 const reset=document.querySelector('#new-round');if(reset)reset.onclick=()=>{if(confirm('Replace this scorecard with a new round? Record your results in TheGrint first.')){state={version:1,startedAt:new Date().toISOString(),results:{},page:0,activeHole:round.holes[0].n};navigate(0)}};
...
async function copyForCoach(){
 ...
 const text=RoundResults.coachReport(round,state.results);
...
function bindTracker(h){
 ...
 const saveResult=()=>{state.activeHole=RoundResults.afterEntry(round.holes,state.activeHole,h.n,before,result());before={...result()};persist();const label=document.querySelector('.active-context');if(label)label.textContent='Playing Hole '+state.activeHole};
