...
## Pocket Caddie redesign (visual acceptance required)
The course briefing, Game Plan, hole recommendation and round debrief use open layouts instead of nested cards. System typography, tabular numerals, cream/green/earth colors and segmented result controls keep the interface lightweight. Club, target and advice lead the hole screen; the tracker remains below them. Summary leads with score and relative-to-par, with Golf Coach export as a secondary action.

A single persistent navigation row contains Previous, the page selector, Summary on hole screens, and Next. The separate “Playing Hole” label still reports the saved active hole even when browsing another hole. Safe-area padding applies on all four edges. Scrolling, touch/swipe exclusions and active-hole rules are unchanged. Cache `caddie-pocket-redesign-1` updates the presentation without changing stored data.

Run all four test suites above. The browser suite additionally checks all hole recommendation details, card removal, recommendation typography, minimum navigation/score touch targets, text contrast, simulated safe-area padding, and layouts at 320/390/844/1280-pixel widths. It tests service-worker updates from the pre-redesign `86b7d9c` release. Set `REVIEW_SCREENSHOT` to a PNG path to capture course, Game Plan, hole and summary review images.

Before merging, validate on an actual iPhone in Safari and installed PWA mode: outdoor readability, one-handed navigation, notch/home-indicator clearance in both orientations, scrolling and score/notes entry with the keyboard open, larger system text, scorecard horizontal scrolling, copy and manual-selection fallback, active-hole return after looking ahead, relaunch persistence, and online update followed by airplane-mode use. Desktop mobile emulation does not establish iPhone acceptance. Do not merge until that review is complete.
