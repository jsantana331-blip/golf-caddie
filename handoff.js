...
const CaddieHandoff = (() => {
 ...
 function show(record,save){
  ...
  function openExternal(){
   // Installed iPhone PWAs can lose activation after async copy. Prefer an explicit link there.
   if(matchMedia('(display-mode: standalone)').matches||navigator.standalone){status.textContent+=' · Switch to ChatGPT or use Open ChatGPT below.';return}
   try{window.open('https://chatgpt.com/','_blank','noopener,noreferrer')}catch{/* The explicit link remains available. */}
   status.textContent+=' · If ChatGPT did not open, use the link below.';
  }
  ...
  async function copy(open){
   if(busy)return;busy=true;find('copy-open').disabled=true;find('just-copy').disabled=true;pending=null;find('ask-fallback').hidden=true;
   const context=CaddieContext.live(record),event=CaddieMemory.event(record);
   try{
    if(!navigator.clipboard?.writeText)throw Error('Clipboard unavailable');
    await navigator.clipboard.writeText(context);
    const saved=log(event);
    if(dialog.isConnected){feedback(saved);if(open)openExternal()}
   }catch{
