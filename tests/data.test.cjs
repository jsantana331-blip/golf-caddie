...
for(const file of execFileSync('git',['ls-files','--cached','--others','--exclude-standard'],{encoding:'utf8'}).trim().split(/\r?\n/).filter(f=>fs.existsSync(f)&&/\.(js|cjs|json|html|css|md)$/.test(f))){
...
const projected=require('../memory.js').courseRound(course,'blue',player);
for(const key of ['course','location','tee','totalYards','rating','slope','mission','holes','courseStyle','grassType','drivingRange','practiceGreen','gamePlan'])assert.deepEqual(projected[key],round[key],key);
assert.equal('clubs' in course,false);assert.equal('weather' in course,false);
assert.deepEqual(player.profile,{});assert.equal(player.handicap,null);assert.deepEqual(player.tendencies,[]);
console.log('Separated player/course data preserves the complete verified V1.2 reference');

assert.deepEqual(player.clubs,round.clubs);assert.equal(player.swingCue,round.swingCue);
